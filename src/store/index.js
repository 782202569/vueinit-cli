...
export const store = {
  ...
  state: {
    ...
    recordingPlayPage: {
      ...
      callId: '', // 录音编号
      recordFileURL: '', // 录音地址
      playType: '', // (我的质检任务-复议任务) 判断是否有复检流程 0表示没有，1表示有
  ...
  mutations: {
    ...
    setRecordingPlayUrl(state, { recordFileURL }) {
      state.recordingPlayPage.recordFileURL = recordFileURL
    },
    setRecordingPlayPage(state, obj) {
      if (obj) {
        ...
        if (obj.recordFileURL) {
          state.recordingPlayPage.recordFileURL = obj.recordFileURL
        } else {
          state.recordingPlayPage.recordFileURL = ''
        }
    ...
    setMenuInfo(state, obj) {
      let menuArr = []
      if (obj) {
        obj.forEach((item) => {
          if (item.menuId === 'a34c89e251b611e89ccd000c2951be64') {
            let value = item.childs.map((val) => {
              if (val.url.includes('/')) {
                val.url = val.url.substr(1)
              }
              return {
                url: val.url,
                menuId: val.menuId,
